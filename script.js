...
// 2. Логіка ліміту: перевірка дати та кількості спроб
function checkDailyLimit() {
    const today = new Date().toLocaleDateString(); // Поточна дата
    const lastDate = localStorage.getItem('task_date');
    let attempts = parseInt(localStorage.getItem('task_attempts')) || 0;

    // Якщо дата змінилася (настав новий день), скидаємо лічильник
    if (lastDate !== today) {
        attempts = 0;
...
// 3. Обробник події при натисканні на кнопку
btn.addEventListener('click', () => {
    let currentAttempts = checkDailyLimit();

    // Якщо ліміт (3) вичерпано
    if (currentAttempts >= 3) {
        alert("Ви вже використали свої 3 спроби на сьогодні! Повертайтеся завтра. ✨");
    ...
    // Збільшення кількості спроб та збереження у пам'ять браузера
    currentAttempts++;
    localStorage.setItem('task_attempts', currentAttempts);

    // Оновлення тексту кнопки
    if (currentAttempts < 3) {
        btn.textContent = `Спробувати ще раз (залишилось: ${3 - currentAttempts})`;
    } else {
        btn.textContent = "Ліміт вичерпано";
...
// Перевірка статусу відразу при завантаженні сторінки
window.onload = () => {
